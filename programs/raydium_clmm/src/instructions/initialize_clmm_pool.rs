...
pub struct InitializeClmmPool<'info> {
    ...
    /// This checks the uniqueness of the pool
    #[account(
    ...
    /// CHECK: Initialize an account to store oracle observations, the account must be created off-chain
    #[account(mut)]
...
pub fn initialize_clmm_pool_handler(
